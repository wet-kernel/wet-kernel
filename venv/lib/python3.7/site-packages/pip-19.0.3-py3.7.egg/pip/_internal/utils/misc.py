...
def call_subprocess(
    ...
    logger.debug("Running job_commands %s", command_desc)
    env = os.environ.copy()
    if extra_environ:
    ...
    try:
        ...
        logger.critical(
            "Error %s while executing job_commands %s", exc, command_desc,
        )
        raise
    all_output = []
    ...
    if proc.returncode and proc.returncode not in extra_ok_returncodes:
        if on_returncode == 'raise':
            if (logger.getEffectiveLevel() > std_logging.DEBUG and
                    not show_stdout):
                logger.info(
                    'Complete output from job_commands %s:', command_desc,
                )
                logger.info(
                    ''.join(all_output) +
                    '\n----------------------------------------'
...
def protect_pip_from_modification_on_windows(modifying_pip):
    ...
    if should_show_use_python_msg:
        ...
        raise CommandError(
            'To modify pip, please run the following job_commands:\n{}'
